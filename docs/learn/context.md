...
1. The provider component will reconcile unrelated to actual changes within the context. We need to use memoizing hooks to ensure that reconciliation of the context provider does not change reference of the context value unnecessarily, or all components consuming the context reconciles as well
2. When the context value actually changes reference, due to a state change, all consumers of the context will reconcile. That means React contexts is an "all or nothing" deal. Components consuming the context reconciles on any change in the context, not by the state they actually consume
3. The component as a function is an amazing concept. It makes a lot of sense to think of a function the returns a description of the UI and when the function is called again the new description is synced with the UI. This is what Reacts reconciler enable. Some years later state management within the reconciler was introduced with hooks, allowing us to fully embrace the function component. Hooks is the most elegant way to introduce state management within the reconciler, but a reconciler is not something you would introduce to make state management more approachable, accessible or predictable in and of itself. As contexts is only about state management there are really no benefits to using a reconciler. There are other concepts, like reactivity, that is more suited for state management
