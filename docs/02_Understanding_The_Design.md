...
## Embracing ergonomic syntax

Since **Impact** embraces object oriented code it only makes sense that it also embraces syntax that makes that paradigm more ergonomic to write. TypeScript has offered [decorators](https://www.typescriptlang.org/docs/handbook/decorators.html) for a long time now, but the official implementation in JavaScript is at [Stage 3](https://github.com/tc39/proposal-decorators). Decorators is a powerful concept which allows us to write more explicit and intenful code.